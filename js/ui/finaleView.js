...
export function renderFinalePhase(finale, handlers, root) {
  ...
  if (finale.phase === 'mengpo') {
    ...
    appendNext(box, '回天 ▸', handlers.onNextPhase);
    ...
    if (d.source) {
      const a = el('a', 'card-source', d.source.label ? `結算取材：${d.source.label}` : `結算取材：《地獄遊記》第${d.source.chapters}回`);
      a.href = d.source.url;
      a.target = '_blank';
      a.rel = 'noopener';
      box.appendChild(a);
  ...
  if (!canvas) {
    ...
    if (typeof canvas.toBlob === 'function') canvas.toBlob((blob) => {
      if (blob) {
        ...
        const file = new File([blob], '地獄遊記親子版-稱號卡.png', { type: 'image/png' });
